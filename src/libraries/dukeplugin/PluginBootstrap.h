...
private:
    ...
    static OfxStatus _mainEntry(const char* action, const void* handle, OfxPropertySetHandle inArgs, OfxPropertySetHandle outArgs) {
        try {
...
public:
    OfxPluginInstance(const char* api, int apiVersion, const char* idendifier, int major, int minor, IPlugin* pPluginInstance) :
        pluginApiHolder(api), pluginIdentifierHolder(idendifier) {
